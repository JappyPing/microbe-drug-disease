...
title('LOOCV-ROC curve','FontSize',16);
xlabel('False Positive Rate','FontSize',16);
ylabel('True Positive Rate','FontSize',16);
legend({'NTSMDA(auc=0.9070)','KATZHMDA(auc=0.8382)','NBLPIHMDA(auc=0.9041)','BiRWMP(auc=0.8637)','BPNNHMDA(auc=0.9281)','HMDA-Pred(auc=0.9589)','LRLSHMDA(auc=0.8909)'},'FontSize',16,'Location','SouthEast');
clear area;
...
overallauc=sum(area);
clear area1;
area2(1,1)=tpr2(1,1)*fpr2(1,1)/2;
for k=2:m*n-pp+1
    area2(1,k)=[tpr2(1,k-1)+tpr2(1,k)]*[fpr2(1,k)-fpr2(1,k-1)]/2;
end
overallauc=sum(area2);
clear area1;
area5(1,1)=tpr5(1,1)*fpr5(1,1)/2;
for k=2:m*n-pp+1
    area7(1,k)=[tpr7(1,k-1)+tpr7(1,k)]*[fpr7(1,k)-fpr7(1,k-1)]/2;
end
overallauc=sum(area7);
