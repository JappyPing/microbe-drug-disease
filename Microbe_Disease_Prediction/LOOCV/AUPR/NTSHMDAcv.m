function NTSHMDAcv(gamadd,gamall,gamma,phi,delte,beta1,beta2)
...
for i=1:nd
    for j=1:nm
        if interaction(i,j)==1
           prediction_score(j,i)=-10000;
        end
    end
end

[ll1,mm1]=size(find(prediction_score>=finalscore));
[ll2,mm2]=size(find(prediction_score>finalscore));
positionNTS(1,cv)=ll2+1+(ll1-ll2-1)/2;
