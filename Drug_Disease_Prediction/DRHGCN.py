...
from Models.DRHGCN import metric_fn
from Models.DRHGCN.utils import init_logger, logger
from Models.DRHGCN.dataloader import CVDataset, DRDataset
...
def train(config, model_cls=DRHGCN):
    ...
    log_dir = os.path.join(f"{config.comment}",f"{model_cls.__name__}")
    ...
    dir1="Evaluate_data/DRHGCN"
    with pd.ExcelWriter(os.path.join(dir1, f"{output_file_name}.xlsx")) as f:
    ...
    print("DRHGCN_avg_auc:",final_metric['auroc'])
    print("DRHGCN_avg_aupr:", final_metric['aupr'])
...
def parse(print_help=False):
    ...
    parser.add_argument("--comment", default="ten_fold_predict_result", type=str, help="experiment name")
